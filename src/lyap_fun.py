...
def compute_CLV(Q: np.ndarray, R: np.ndarray) -> np.ndarray:
    """
...
def qr_mgs(A: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
...
#def compute_ICLE(jacobian_history: np.ndarray, CLV_history: np.ndarray) -> np.ndarray:
    """
    ...
    Parameters
    ----------
    jacobian_history : ndarray, shape (n, n, nt)
        Time history of Jacobian matrices J(t).
    ...
    """
    VTJV = np.einsum("ikt, ijt, jkt -> kt", CLV_history, jacobian_history, CLV_history) # (m,nt)
    return VTJV

#def compute_jacobian_time_history(jacobian_function: Callable, solution: np.ndarray, time: np.ndarray, *args) -> np.ndarray:
    """
    Compute the time history of Jacobian matrices along a trajectory.

    Parameters
    ----------
    jacobian_function : Callable
        Function that returns the Jacobian at a given time/state:
        ``jacobian_function(t: float, x: ndarray, *args) -> ndarray (n, n)``.
    solution : ndarray, shape (n, nt)
    ...
    *args : Any
    ...
    Returns
    -------
    ndarray, shape (n, n, nt)
        Time history of Jacobian matrices ``J(t_i)``.
    """
    nt = len(time)
    n = np.shape(solution)[0]
    jacobian_history = np.zeros((n, n, nt))

    for i in range(nt):
        jacobian_history[:, :, i] = jacobian_function(time[i], solution[:, i], *args)

    return jacobian_history
